...
def odometry_callback(msg):
    ...
    if t1 != 0 and t2 > t1:
        theta = theta + (omega ) * (t2-t1) + np.random.normal(0, 0.02)
        particle_x = particle_x + (v ) * np.cos(theta) * (t2-t1) + np.random.normal(0, 0.02)
        particle_y = particle_y + (v) * np.sin(theta) * (t2-t1) + np.random.normal(0, 0.02)
    t1 = t2
    i+=1
    if (i==10):
        dead_time.append(t2)
...
def initialization():
    ...
    scatter_amcl = ax.scatter([], [], color='red', s=5, label='Ground Truth')
