...
# Example usage - 17
x_particle, y_particle, theta_particle, time_particle = interpolate_csv('particle_csv', 467 )
x_amcl, y_amcl, theta_amcl, time_amcl = interpolate_csv('amcl_csv', 467)
x_dead, y_dead, theta_dead, time_dead = interpolate_csv('dead', 467)
...
common_time = np.arange(
    ...
    1/467 *(min(time_amcl.max(), time_particle.max())-max(time_amcl.min(), time_particle.min()))  # Use a smaller time step, e.g., 0.1, for smoother interpolation
)
