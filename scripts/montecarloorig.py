...
class ParticleFilter(object):
    ...
    def update_particle(self, laser_scan, v, omega):
        ...
        if (v >= 0.001 or v <= -0.001 or omega >= 0.01 or omega <= -0.01): 
            ...
            #[self.likelihood_field_range_finder_model(particle, actual_ranges) for particle in self.particles]
    ...
    def likelihood_field_range_finder_model(self, particle, actual_ranges):
        q = 1.0
        
        angles = np.radians(np.arange(0, 360, 4))
        
        cos_angles = np.cos(angles + particle.theta)
        sin_angles = np.sin(angles + particle.theta)
        
        x1 = particle.x + actual_ranges * cos_angles
        y1 = particle.y + actual_ranges * sin_angles
        x_indices, y_indices = self.conversao_pixeis(x1, y1)
        valid_distances = self.mapa[y_indices, x_indices]

        non_zmax_indices = np.where(actual_ranges != 3.5)
        valid_distances_non_zmax = valid_distances[non_zmax_indices]
        
        q *= np.prod(0.1 * self.prob(valid_distances_non_zmax, 30))
        
        particle.weight = q

    def prob(self, dist, sigma2_hit):
        return np.exp(-0.5 * (dist ** 2) / sigma2_hit) / np.sqrt(2 * np.pi * sigma2_hit)
    ...
    def resample(self, novas_particulas):
        ...
        if novas_particulas > 0:
            for i in range(novas_particulas):
                ...
                x,y = self.conversao_metros(self.position_particula[h,1], self.position_particula[h,0])
